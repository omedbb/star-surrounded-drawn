...
@property (nonatomic, assign) BOOL hasShownBuddyBuildIntroInstructions;
@property (nonatomic, assign) BOOL hasShownBuddyBuildFinalInstructions;
@property (nonatomic, strong) BuddyBulidDemoLetsCauseACrashViewController* causeACrashVC;
@end
...
- (void)viewDidLoad
{
  ...
  [[NSNotificationCenter defaultCenter] addObserverForName:@"BUDDYBUILD_FEEDBACK_SENT" object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
      BuddyBulidDemoLetsCauseACrashViewController* vc = [[BuddyBulidDemoLetsCauseACrashViewController alloc] initWithNibName:@"BuddyBulidDemoLetsCauseACrashViewController" bundle:nil];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (![userDefaults boolForKey:@"has_sent_feedback"] && ![userDefaults boolForKey:@"has_sent_crashreport"]) {
        if (self.hasShownBuddyBuildIntroInstructions) {
            return;
        }

        self.hasShownBuddyBuildIntroInstructions = YES;        
        [appDelegate sendNotification:@"notifyDemoAppOpened" callback:nil];
        BuddyBuildDemoIntroductionViewController* bbDemoIntroductionViewController = [[BuddyBuildDemoIntroductionViewController alloc] initWithNibName:@"BuddyBuildDemoIntroductionViewController" bundle:nil];
        bbDemoIntroductionViewController.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;

        [self presentViewController:bbDemoIntroductionViewController animated:YES completion:^{
    ...
    if ([userDefaults boolForKey:@"has_sent_feedback"] && [userDefaults boolForKey:@"has_sent_crashreport"]) {
        if (self.hasShownBuddyBuildFinalInstructions) {
            return;
        }

        if ([userDefaults boolForKey:@"has_show_final_instructions"]) {
            return;
        }

        self.hasShownBuddyBuildFinalInstructions = YES;
        [userDefaults setBool:YES forKey:@"has_show_final_instructions"];
        [userDefaults synchronize];
        //[appDelegate sendNotification:@"notifyDemoAppCrashReportReceived"];

        BuddyBuildDemoAppHasCrashedViewController* vc = [[BuddyBuildDemoAppHasCrashedViewController alloc] initWithNibName:@"BuddyBuildDemoAppHasCrashedViewController" bundle:nil];
    ...
    /*
    if ([userDefaults objectForKey:@"has_shown_screenshot_tutorial"]) {
        appDelegate.screenshottingAllowed = true;
        return;
    };*/

    /*
    [userDefaults setObject:[NSNumber numberWithBool:YES] forKey:@"has_shown_screenshot_tutorial"];
